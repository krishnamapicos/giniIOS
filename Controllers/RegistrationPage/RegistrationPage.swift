...
class RegistrationPage: UIViewController,RMPScrollingMenuBarControllerDelegate,UITextFieldDelegate{
    ...
    @IBOutlet weak var goToLogInPageCall: UIButton!

    @IBAction func signUpAPICall(sender: AnyObject) {
        
        if validText(){
            print("valid")
           
            emailText.layer.borderColor = UIColor.clearColor().CGColor
    ...
    func registrationAPICall(){
            ...
            let strParams =  "&email_address=" + emailText.text!  +  "&password=" + passwordText.text!
            let data = strParams.dataUsingEncoding(NSUTF8StringEncoding)! as NSData
            
            print("params\(params)")
            
            
            let url:NSURL = NSURL(string:"http://103.1.172.70/GINI/gini/index.php/REST/Auth/signup")!
